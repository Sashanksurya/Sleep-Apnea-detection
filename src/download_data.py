...
def load_record(record_name):
    ...
    # ── FIX: equalise lengths (some records differ by 1 sample per channel) ──
    length = min(len(ecg), len(spo2))
    ecg    = ecg[:length]
    spo2   = spo2[:length]

    # Replace NaN/Inf with channel mean so downstream code never sees bad values
    for arr in (ecg, spo2):
        bad = ~np.isfinite(arr)
        if bad.any():
            arr[bad] = np.nanmean(arr[~bad]) if not bad.all() else 0.0
...
def segment_signals(ecg, spo2, samples, labels, fs, window_sec=60):
    ...
    for start, label in zip(samples, labels):
        ...
        # Equalise window lengths (defensive — channels should already match)
        wlen = min(len(e), len(s), window)
        e, s = e[:wlen], s[:wlen]
        if wlen < window:   # window too short — pad with zeros
            e = np.pad(e, (0, window - wlen))
            s = np.pad(s, (0, window - wlen))
